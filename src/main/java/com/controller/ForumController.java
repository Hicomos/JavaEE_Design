    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(forum !=null){
            ...
            //级联表 接取用户
            //级联表
            JiequyonghuEntity jiequyonghu = jiequyonghuService.selectById(forum.getJiequyonghuId());
            if(jiequyonghu != null){
...
, "usersId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            //级联表 发布用户
            //级联表
            FabuyonghuEntity fabuyonghu = fabuyonghuService.selectById(forum.getFabuyonghuId());
            if(fabuyonghu != null){
    ...
    */
    ...
    public R save(@RequestBody ForumEntity forum, HttpServletRequest request){
        ...
        if(false)
            ...
            forum.setJiequyonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        else if("发布用户".equals(role))
            forum.setFabuyonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        Wrapper<ForumEntity> queryWrapper = new EntityWrapper<ForumEntity>()
            ...
            .eq("jiequyonghu_id", forum.getJiequyonghuId())
            .eq("fabuyonghu_id", forum.getFabuyonghuId())
...
//                            forumEntity.setJiequyonghuId(Integer.valueOf(data.get(0)));   //接取用户 要改的
//                            forumEntity.setFabuyonghuId(Integer.valueOf(data.get(0)));   //发布用户 要改的
